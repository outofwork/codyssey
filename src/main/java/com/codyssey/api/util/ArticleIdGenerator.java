...
/**
 * Custom ID generator for Article entities
 * <p>
 * Generates IDs in format: ART-100001, ART-100002, etc.
...
public class ArticleIdGenerator implements IdentifierGenerator {
    ...
    /**
     * Validates if a string is a valid Article ID
     *
     * @param id The ID to validate
     * @return true if valid, false otherwise
     */
    public static boolean isValidArticleId(String id) {
        if (id == null) {
            return false;
        }
    ...
    public Serializable generate(SharedSessionContractImplementor session, Object object) {
        ...
        try {
            // Get the maximum existing number for articles (only numeric IDs)
            String sql = "SELECT MAX(CAST(SUBSTRING(id, 5) AS INTEGER)) FROM articles WHERE id ~ '^ART-\\d{6}$'";
            ...
            throw new RuntimeException("Failed to generate Article ID", e);
        } finally {
            try {
