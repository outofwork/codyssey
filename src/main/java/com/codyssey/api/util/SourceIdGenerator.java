...
/**
 * Custom ID generator for Source entities
 * <p>
 * Generates IDs in format: SRC-100001, SRC-100002, etc.
...
public class SourceIdGenerator implements IdentifierGenerator {
    ...
    /**
     * Validates if a string is a valid Source ID
     *
     * @param id The ID to validate
     * @return true if valid, false otherwise
     */
    public static boolean isValidSourceId(String id) {
        if (id == null) {
            return false;
        }
        return id.matches("^SRC-\\d{6}$");
    }
    ...
    public Serializable generate(SharedSessionContractImplementor session, Object object) {
        ...
        try {
            // Get the maximum existing number for sources (only numeric IDs)
            String sql = "SELECT MAX(CAST(SUBSTRING(id, 5) AS INTEGER)) FROM sources WHERE id ~ '^SRC-\\d{6}$'";
            ...
            throw new RuntimeException("Failed to generate Source ID", e);
        } finally {
            try {
